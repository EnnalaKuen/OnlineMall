...
import { checkOut, submitOrder } from '@/api/order'
export default {
  name: 'PayIndex',
  data () {
    return {
  ...
  computed: {
    // 完整的地址拼接
    longAddress () {
      return this.selectAddress.region.city + ',' + this.selectAddress.region.province + ',' + this.selectAddress.region.region + ',' + this.selectAddress.detail
    },
    mode () {
  ...
  methods: {
    async getAddress () {
      ...
      this.selectAddress = res.data.list[3]
    },
    async getCheckOut () {
      // 购物车付款模式
      if (this.mode === 'cart') {
        const res = await checkOut(this.mode, { cartIds: this.cartIds })
        this.order = res.data.order
        this.personal = res.data.personal
      } else if (this.mode === 'buyNow') {
        // 立即购买付款模式
        const res = await checkOut(this.mode, { goodsId: this.goodsId, goodsNum: this.goodsNum, goodsSkuId: this.goodsSkuId })
        this.order = res.data.order
        this.personal = res.data.personal
      }
    },
    async submit () {
      ...
      if (this.mode === 'cart') {
        ...
        const res = await submitOrder(this.mode, { goodsId: this.goodsId, goodsNum: this.goodsNum, goodsSkuId: this.goodsSkuId, remark: this.remark })
        console.log(res)
        this.$toast.success('提交订单成功')
        this.$router.push('/myOrder')
      }
